...
export const oauthConnect = async ({
  ...
  const accessToken = getToken()
  const axiosInstance =
    accessToken !== null ? privateAxiosInstance : publicAxiosInstance
...
export const createEntity = async <T>(iri: string, data: any): Promise<T> => {
  return privateAxiosInstance
    .post<T>(iri, data)
    .then((response) => response.data)
}
...
export const updateEntity = async <T>(iri: string, data: any): Promise<T> => {
  let config = {
    headers: {
...
export const getListEntities = async <T>(iri: string): Promise<T[]> => {
  return getHydraList<T>(iri).then((response) => response['member'])
}
